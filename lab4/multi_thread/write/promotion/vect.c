...
double int_write(void* buf, size_t size_in_bytes) {
    ...
    for (size_t try = 0; try < TRY_COUNT; ++try) {
        ...
        for (size_t i = 0; i < int_el_count; i += 32) {
            ...
            _mm512_storeu_epi32(&int_buf[i + 16], _mm512_set1_epi32(1));
        }
...
double float_write(void* buf, size_t size_in_bytes) {
    ...
    for (size_t try = 0; try < TRY_COUNT; ++try) {
        ...
        for (size_t i = 0; i < float_el_count; i += 32) {
            ...
            _mm512_storeu_ps(&float_buf[i + 16], _mm512_set1_ps(1.0f));
        }
...
double double_write(void* buf, size_t size_in_bytes) {
    ...
    for (size_t try = 0; try < TRY_COUNT; ++try) {
        ...
        for (size_t i = 0; i < double_el_count; i += 16) {
            ...
            _mm512_storeu_pd(&double_buf[i + 8], _mm512_set1_pd(1.0));
        }
