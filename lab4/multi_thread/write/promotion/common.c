...
double char_write(void* buf, size_t size_in_bytes) {
    ...
    fprintf(stdout, "Buffer have values in index 0, 10, 20: %d, %d, %d\n", char_buf[0], char_buf[10], char_buf[20]);
    fprintf(stdout, "Buffer initialized with %zu elements\n", char_el_count);
...
void* thread_func(void* args) {
    ...
    switch (data_type) { 
        ...
        case CHAR: {
            gbps = char_write(buf, size_in_bytes);
            break;
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    switch (in_data_type) { 
        ...
        case CHAR: {
            gbps = char_write(buf, size_in_bytes_per_thread);
            break;
        }
    }
