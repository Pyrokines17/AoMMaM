...
double int_copy(void* buf, void* buf2, size_t size_in_bytes) {
    ...
    for (size_t try = 0; try < TRY_COUNT; ++try) {    
        ...
        for (size_t i = 0; i < int_el_count; i += 32) {
            ...
            __m512i v2 = _mm512_loadu_si512(&int_buf[i + 16]);
            _mm512_storeu_si512(&int_buf2[i + 16], v2);
        }
...
double float_copy(void* buf, void* buf2, size_t size_in_bytes) {
    ...
    for (size_t try = 0; try < TRY_COUNT; ++try) {    
        ...
        for (size_t i = 0; i < float_el_count; i += 32) {
            __m512 v = _mm512_loadu_ps(&float_buf[i]);
            _mm512_storeu_ps(&float_buf2[i], v);
            __m512 v2 = _mm512_loadu_ps(&float_buf[i + 16]);
            _mm512_storeu_ps(&float_buf2[i + 16], v2);
        }
...
double double_copy(void* buf, void* buf2, size_t size_in_bytes) {
    ...
    for (size_t try = 0; try < TRY_COUNT; ++try) {    
        ...
        for (size_t i = 0; i < double_el_count; i += 16) {
            __m512d v = _mm512_loadu_pd(&double_buf[i]);
            _mm512_storeu_pd(&double_buf2[i], v);
            __m512d v2 = _mm512_loadu_pd(&double_buf[i + 8]);
            _mm512_storeu_pd(&double_buf2[i + 8], v2);
        }
