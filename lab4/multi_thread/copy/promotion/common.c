...
double int_copy(void* buf, void* buf2, size_t size_in_bytes) {
    ...
    for (size_t try = 0; try < TRY_COUNT; ++try) {    
        ...
        for (size_t i = 0; i < int_el_count; i += 4) {
            int_buf2[i] = int_buf[i];
            int_buf2[i + 1] = int_buf[i + 1];
            int_buf2[i + 2] = int_buf[i + 2];
            int_buf2[i + 3] = int_buf[i + 3];
        }
...
double float_copy(void* buf, void* buf2, size_t size_in_bytes) {
    ...
    for (size_t try = 0; try < TRY_COUNT; ++try) {    
        ...
        for (size_t i = 0; i < float_el_count; i += 4) {
            float_buf2[i] = float_buf[i];
            float_buf2[i + 1] = float_buf[i + 1];
            float_buf2[i + 2] = float_buf[i + 2];
            float_buf2[i + 3] = float_buf[i + 3];
        }
...
double double_copy(void* buf, void* buf2, size_t size_in_bytes) {
    ...
    for (size_t try = 0; try < TRY_COUNT; ++try) {    
        ...
        for (size_t i = 0; i < double_el_count; i += 4) {
            double_buf2[i] = double_buf[i];
            double_buf2[i + 1] = double_buf[i + 1];
            double_buf2[i + 2] = double_buf[i + 2];
            double_buf2[i + 3] = double_buf[i + 3];
        }
...
double char_copy(void* buf, void* buf2, size_t size_in_bytes) {
    char* char_buf = (char*)buf;
    char* char_buf2 = (char*)buf2;
    ...
    for (size_t try = 0; try < TRY_COUNT; ++try) {    
        ...
        for (size_t i = 0; i < char_el_count; i += 4) {
            char_buf2[i] = char_buf[i];
            char_buf2[i + 1] = char_buf[i + 1];
            char_buf2[i + 2] = char_buf[i + 2];
            char_buf2[i + 3] = char_buf[i + 3];
        }
    ...
    fprintf(stdout, "Buffer have values in idex 0, 10, 20: %d, %d, %d\n", char_buf2[0], char_buf2[10], char_buf2[20]);
    fprintf(stdout, "Buffer initialized with %zu elements\n", char_el_count);
...
void* thread_func(void* args) {
    ...
    switch (data_type) { 
        ...
        case FLOAT: {
            gbps = float_copy(buf, buf2, size_in_bytes);
            break;
        }
        case DOUBLE: {
            gbps = double_copy(buf, buf2, size_in_bytes);
            break;
        }
        case CHAR: {
            gbps = char_copy(buf, buf2, size_in_bytes);
            break;
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    void* buf2 = malloc(size_in_bytes_per_thread);
    ...
    switch (in_data_type) { 
        ...
        case FLOAT: {
            gbps = float_copy(buf, buf2, size_in_bytes_per_thread);
            break;
        }
        case DOUBLE: {
            gbps = double_copy(buf, buf2, size_in_bytes_per_thread);
            break;
        }
        case CHAR: {
            gbps = char_copy(buf, buf2, size_in_bytes_per_thread);
            break;
        }
    }
