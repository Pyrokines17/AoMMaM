...
double int_copy(void* buf, void* buf2, size_t size_in_bytes) {
    ...
    for (size_t try = 0; try < TRY_COUNT; ++try) {    
        ...
        for (size_t i = 0; i < int_el_count; i += 32) {
            ...
            __m512i v2 = _mm512_stream_load_si512(&int_buf[i + 16]);
            _mm512_stream_si512(&int_buf2[i + 16], v2);
        }
...
int main(int argc, char* argv[]) {
    ...
    void* buf = _mm_malloc(size_in_bytes_per_thread, 64);
    ...
    void* buf2 = _mm_malloc(size_in_bytes_per_thread, 64);
