...
double int_read(void* buf, size_t size_in_bytes) {
    ...
    for (size_t try = 0; try < TRY_COUNT; ++try) {    
        ...
        for (size_t i = 0; i < int_el_count; i += 8) {
            __m128i v = _mm_loadu_si128((__m128i_u*)&int_buf[i]);
            sum_vec = _mm_add_epi32(sum_vec, v);
            __m128i v2 = _mm_loadu_si128((__m128i_u*)&int_buf[i + 4]);
            sum_vec = _mm_add_epi32(sum_vec, v2);
        }
        
        clock_gettime(CLOCK_MONOTONIC, &end);

        s += _mm_extract_epi32(sum_vec, 0);
        s += _mm_extract_epi32(sum_vec, 1);
        s += _mm_extract_epi32(sum_vec, 2);
        s += _mm_extract_epi32(sum_vec, 3);
...
double double_read(void* buf, size_t size_in_bytes) {
    ...
    for (size_t try = 0; try < TRY_COUNT; ++try) {
        ...
        for (size_t i = 0; i < double_el_count; i += 4) {
            ...
            sum_vec = _mm_add_pd(sum_vec, v2);
        }
        
        clock_gettime(CLOCK_MONOTONIC, &end);

        sum_vec = _mm_hadd_pd(sum_vec, sum_vec);
        s += _mm_cvtsd_f64(sum_vec);
