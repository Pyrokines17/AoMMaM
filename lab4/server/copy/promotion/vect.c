...
double int_copy(void* buf, void* buf2, size_t size_in_bytes) {
    ...
    for (size_t try = 0; try < TRY_COUNT; ++try) {    
        ...
        for (size_t i = 0; i < int_el_count; i += 8) {
            __m128i data = _mm_loadu_si128((__m128i_u*)&int_buf[i]);
            _mm_storeu_si128((__m128i_u*)&int_buf2[i], data);
            __m128i data2 = _mm_loadu_si128((__m128i_u*)&int_buf[i + 4]);
            _mm_storeu_si128((__m128i_u*)&int_buf2[i + 4], data2);
        }
...
double float_copy(void* buf, void* buf2, size_t size_in_bytes) {
    ...
    for (size_t try = 0; try < TRY_COUNT; ++try) {    
        ...
        for (size_t i = 0; i < float_el_count; i += 8) {
            __m128 v = _mm_loadu_ps(&float_buf[i]);
            _mm_storeu_ps(&float_buf2[i], v);
            __m128 v2 = _mm_loadu_ps(&float_buf[i + 4]);
            _mm_storeu_ps(&float_buf2[i + 4], v2);
        }
...
double double_copy(void* buf, void* buf2, size_t size_in_bytes) {
    ...
    for (size_t try = 0; try < TRY_COUNT; ++try) {    
        ...
        for (size_t i = 0; i < double_el_count; i += 4) {
            __m128d v = _mm_loadu_pd(&double_buf[i]);
            _mm_storeu_pd(&double_buf2[i], v);
            __m128d v2 = _mm_loadu_pd(&double_buf[i + 2]);
            _mm_storeu_pd(&double_buf2[i + 2], v2);
        }
