...
double int_write(void* buf, size_t size_in_bytes) {
    ...
    for (size_t try = 0; try < TRY_COUNT; ++try) {
        ...
        for (size_t i = 0; i < int_el_count; i += 8) {
            _mm_stream_si128(&int_buf[i], _mm_set1_epi32(1));
            _mm_stream_si128(&int_buf[i + 4], _mm_set1_epi32(1));
        }
...
double float_write(void* buf, size_t size_in_bytes) {
    ...
    for (size_t try = 0; try < TRY_COUNT; ++try) {
        ...
        for (size_t i = 0; i < float_el_count; i += 8) {
            _mm_storeu_ps(&float_buf[i], _mm_set1_ps(1.0f));
            _mm_storeu_ps(&float_buf[i + 4], _mm_set1_ps(1.0f));
        }
...
double double_write(void* buf, size_t size_in_bytes) {
    ...
    for (size_t try = 0; try < TRY_COUNT; ++try) {
        ...
        for (size_t i = 0; i < double_el_count; i += 4) {
            _mm_storeu_pd(&double_buf[i], _mm_set1_pd(1.0));
            _mm_storeu_pd(&double_buf[i + 2], _mm_set1_pd(1.0));
        }
...
void* thread_func(void* args) {
    ...
    switch (data_type) { 
        ...
        case FLOAT: {
            gbps = float_write(buf, size_in_bytes);
            break;
        }
        case DOUBLE: {
            gbps = double_write(buf, size_in_bytes);
            break;
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    switch (in_data_type) { 
        ...
        case FLOAT: {
            gbps = float_write(buf, size_in_bytes_per_thread);
            break;
        }
        case DOUBLE: {
            gbps = double_write(buf, size_in_bytes_per_thread);
            break;
        }
    }
