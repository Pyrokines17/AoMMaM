...
new_code = 'asm("nop");'*318

for i in range(318, border):
    mod_code = code.replace("int nop_count = 1;", f"int nop_count = {i};")
    ...
    comp_res = subprocess.run(["gcc", temp_c_name, "-o", output_exe], capture_output=True, text=True)
